...
def get_st_diagram(med_folder_name, error_filter=None):
    ...
    if error_filter != None:
        st_diagram[st_error > np.mean(st_error) +
                   np.std(st_error) * error_filter] = np.nan
        st_diagram = fill_nans(st_diagram)

    return st_diagram[:3000]
...
def point_through_time(med_folder_name, point):
    ...
    ax.set_ylabel(f'"Altura"', fontsize=16)
...
def error_filter_analysis(med_folder_name, start=3, interval=1):
    ...
    # ax1.set_title('Diagrama filtrado a $3 \sigma$', fontsize=22, pad=20)
    # ax1.imshow(st_diagram_tmp, cmap=cmap)

    # ax2.set_title('Diagrama con interpolación', fontsize=22, pad=20)
    ...
    # ax1.set_yticks([])
    # plt.savefig(
    # f'../../Informes/Labo7_presentacion/figs/error_interp.pdf',
    # bbox_inches='tight',
    # transparent=True)
    # plt.show()
    ...
    axes = axes.flatten()
    cmap.set_bad('black')
    axes[0].set_title('Completa', fontsize=16, pad=10)
    axes[0].imshow(st_diagram, cmap=cmap)
    for i in range(start, start + 5, interval):
        st_diagram_tmp = st_diagram.copy()
        st_diagram_tmp[st_error > np.mean(st_error) +
                       np.std(st_error) * i] = np.nan

        axes[i - start + 1].set_title('NaN si $ err > \\overline{err} +' +
                                      str(i) + '\sigma $',
                                      fontsize=16,
                                      pad=10)
        axes[i - start + 1].imshow(st_diagram_tmp, cmap=cmap)

    axes[0].set_xlim(1400, 1520)
    axes[0].set_ylim(1850, 1970)
    axes[0].set_xticks([])
    axes[0].set_yticks([])
    plt.savefig(f'../../Informes/Labo7_presentacion/figs/error_analysis.pdf',
                bbox_inches='tight',
                transparent=True)
    plt.show()
...
def main():
    ...
    # med_folder_name = 'MED40 - Oscilones a full - 0902'
    med_folder_name = 'MED30 - Subida en voltaje, NOTA - 0902'
    # med_folder_name = 'MED39 - Subida en voltaje - 0902'

    st_diagram = get_st_diagram(med_folder_name, error_filter=5)
    # st_diagram = np.diff(st_diagram, axis=0)
    plt.imshow(st_diagram)#, clim=(-1, 1))
    plt.colorbar()
    plt.show()
