...
class FTP():
    ...
    def _get_image_thresholded(self, image):
        ...
        idx_max = signal.argrelextrema(
            y, np.greater)[0]  # determina los máximos locales
    ...
    def export(self):
        f = h5py.File(self.hdf5_folder + 'FTP.hdf5',
                      'w',
                      driver='mpio',
                      comm=MPI.COMM_WORLD)
    ...
    def _get_mask_and_export(self, masks_grp):
        ...
        logging.info(
            f'FTP: Guardando atributos del centro y los radios del anillo')
